...
CREATE TABLE IF NOT EXISTS `pay_transaction` (
  ...
  `method_id` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '支付方式id，可以用来识别支付，如：支付宝、微信、Paypal等',
  ...
  `client_ip` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '发起支付的ip',
  ...
  `notify_status` TINYINT NOT NULL DEFAULT 0 COMMENT '0未通知 1通知成功 2通知不成功',
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `uniq_app_order` (`app_id` ASC, `order_id` ASC) VISIBLE,
  INDEX `idx_method` (`method_id` ASC) VISIBLE,
  INDEX `idx_trade_no` (`trade_no` ASC) VISIBLE,
  UNIQUE INDEX `uniq_trancationno` (`transaction_no` ASC) VISIBLE)
ENGINE = InnoDB
...
CREATE TABLE IF NOT EXISTS `pay_transaction_extension` (
  ...
  PRIMARY KEY (`id`),
  INDEX `idx_transaction` (`transaction_no` ASC, `pay_status` ASC) VISIBLE,
  UNIQUE INDEX `uniq_code` (`transaction_code` ASC) VISIBLE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COMMENT = '交易扩展表';
...
CREATE TABLE IF NOT EXISTS `pay_transaction_log` (
  ...
  PRIMARY KEY (`id`),
  INDEX `idx_app_order` (`app_id` ASC, `order_id` ASC) VISIBLE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COMMENT = '交易日志表';
...
CREATE TABLE IF NOT EXISTS `pay_repeat_transaction` (
  ...
  `method_id` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '支付方式',
  ...
  PRIMARY KEY (`id`),
  INDEX `idx_transaction` (`app_id` ASC, `transaction_no` ASC) VISIBLE,
  INDEX `idx_method` (`method_id` ASC) VISIBLE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COMMENT = '记录重复支付';
...
-- Table 第三方异步通知日志表
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `pay_thirdnotify_log` (
  `id` BIGINT UNSIGNED NOT NULL,
  `transaction_no` VARCHAR(64) NOT NULL COMMENT '系统唯一识别交易号',
...
-- Table 通知对应应用支付结果
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `pay_notifyapp` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `app_id` VARCHAR(32) NOT NULL COMMENT '应用id',
  ...
  `total_fee` INT UNSIGNED NOT NULL DEFAULT 0,
  `scale` TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '小数位数',
  ...
  `app_confirm_time` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '应用通知确认时间',
  ...
  PRIMARY KEY (`id`),
  INDEX `uniq_transaction` (`transaction_no` ASC) VISIBLE,
  INDEX `idx_app` (`app_id` ASC, `notify_type` ASC, `notify_status` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `pay_refund` (
  ...
  `refund_type` TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '退款类型；0:业务退款；1:重复退款；3:补偿性质退款',
  ...
  `request_ip` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '请求源ip',
  `create_at` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '创建时间',
  `update_at` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '更新时间',
  PRIMARY KEY (`id`))
ENGINE = InnoDB
