...
- 第①步：索引过程，需要被搜索到源数据被索引(动词)到搜索引擎中，并建立索引(名词)
- 第②步：查询过程，用户输入关键字(Query)，搜索引擎分析Query并返回数据
...
分析就是文档(Doc)文本标准化并把文档(Doc)转换成词条(Term)的过程。搜索引擎ES分析过程的实现依赖于分词器。分析器基本组成：
...
这里提到了「倒排索引」、「文档(Doc)」、「词条(Term)」、「召回」名词的概念：
